...
**Leaderlog History** — Retroactively classifies every assigned leader slot as forged, slot battle, or missed. Uses Koios pool_blocks for pool-specific forge detection and local chain data for battle detection. Runs as a resumable background job (daemon) or CLI command with time estimate. Note: only slot battles (same slot) are detectable — height battles (different slot, VRF tiebreak) require external orphan data.
